...
--Question 1
--Our goal is to get the city Duluth or Dallas because they make the most
...
--Question 2 
--To check the average it is 8.25/8= 1.03 
...
WHERE priceUSD >= (SELECT AVG (priceUSD) 
		...
		DESC; 
--Question 3
--Puts the total in accending order 
SELECT Orders.totalUSD, Orders.pid, Customers.name 
FROM Customers 
INNER JOIN Orders ON Customers.cid= Orders.cid 
ORDER BY Orders.totalUSD 
ASC;

--Question 4
--using Coalesce 
SELECT Customers.name
SUM COALESCE (Orders.qty,1) 
FROM Customers 
INNER JOIN Customers.cid = Orders.cid 
GROUP BY Customers.name 
ORDER BY Customers.name 
ASC; 

--Question 5 
--Agents in newark with names of products ordered and sold 
SELECT Agents.name, Products.name, Customers.name 
FROM Orders
...
SELECT 
FROM (
SELECT (Orders* Orders.qty * Products.priceUSD * (1-(discount/100)))
AS AccurateUSD 
FROM Orders 
INNER JOIN Products ON Orders.pid = Products.pid 
INNER JOIN Customers ON Orders.cid = Customers.cid)
...
-- Question 7 
-- The difference is that a LEFT OUTER JOIN only returns everything on the left side of the table when meeting the condition. The RIGHT INNER JOIN is when it takes everything on the right side and all conditions on right side are meet. 
-- SELECT Customers.cid, Orders.ordno 
-- FROM Customers 
